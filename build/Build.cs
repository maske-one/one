...
class Build : NukeBuild
{
    ...
    AbsolutePath DistributionDirectory => PublishDirectory / @"one" / @"dist";
    ...
    Target PushGhPages => _ => _
        ...
        .Executes(() =>
        {
            if (PAT == null)
            {
                ...
                GitTasks.Git("push -f  https://github.com/maske-one/one.git gh-pages", DistributionDirectory);
            }
            else
            {
                GitTasks.Git(@"config --global user.name maske-one", DistributionDirectory);
                GitTasks.Git(@"config --global user.email s@maske.one", DistributionDirectory);
                GitTasks.Git("init", DistributionDirectory);
                GitTasks.Git("checkout -b gh-pages", DistributionDirectory);
                GitTasks.Git("add -A", DistributionDirectory);
                GitTasks.Git($"commit -m \"commit ver {GitVersion.FullSemVer}\"", DistributionDirectory);
